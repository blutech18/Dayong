...
import {
  Users, UserCheck, UserX, HeartHandshake, CheckCircle2, XCircle,
  TrendingUp, TrendingDown, Wallet, PiggyBank, ArrowRight, Calendar,
...
import {
  dashboardStats, monthlyCollections, monthlyExpenses,
  assistanceDistribution, members, assistanceRequests, collectionEvents,
  announcements, formatPHP, formatDate, initialsOf,
} from "@/lib/mock-data";
...
export const Route = createFileRoute("/_shell/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Overview of members, collections, assistance and financial health." },
    ],
  }),
...
function DashboardPage() {
  ...
  const pending = assistanceRequests
    .filter((r) => r.status === "pending" || r.status === "under_review")
    .slice(0, 4);
  const upcoming = collectionEvents.filter((e) => e.status === "upcoming").slice(0, 3);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Total members" value={String(dashboardStats.totalMembers)} delta={4.2} deltaLabel="vs last month" icon={Users} tone="primary" />
        <StatCard label="Active members" value={String(dashboardStats.activeMembers)} delta={2.1} deltaLabel="new this month" icon={UserCheck} tone="success" />
        <StatCard label="Pending assistance" value={String(dashboardStats.pendingAssistance)} delta={-8} deltaLabel="vs last week" icon={HeartHandshake} tone="warning" />
        <StatCard label="Cash balance" value={formatPHP(dashboardStats.balance)} delta={5.6} deltaLabel="net this month" icon={PiggyBank} tone="secondary" />
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Monthly collections" value={formatPHP(dashboardStats.monthlyCollections)} delta={-3.4} deltaLabel="vs last month" icon={Wallet} tone="primary" />
        <StatCard label="Monthly expenses" value={formatPHP(dashboardStats.monthlyExpenses)} delta={1.8} deltaLabel="vs last month" icon={TrendingDown} tone="danger" />
        <StatCard label="Approved assistance" value={String(dashboardStats.approvedAssistance)} delta={12} deltaLabel="this quarter" icon={CheckCircle2} tone="success" />
        <StatCard label="Rejected assistance" value={String(dashboardStats.rejectedAssistance)} delta={-2} deltaLabel="this quarter" icon={XCircle} tone="muted" />
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 xl:col-span-2">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-muted-foreground/50" />Expected</span>
            </div>
          </div>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyCollections} margin={{ left: -20, right: 8, top: 8, bottom: 0 }}>
                <defs>
                ...
                />
                <Area type="monotone" dataKey="expected" stroke="var(--color-muted-foreground)" strokeDasharray="4 4" fill="transparent" strokeWidth={1.5} activeDot={{ r: 4 }} />
                <Area type="monotone" dataKey="collected" stroke="var(--color-primary)" fill="url(#collectedGrad)" strokeWidth={2} activeDot={{ r: 5, stroke: "var(--color-background)", strokeWidth: 2 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyExpenses} margin={{ left: -20, right: 8, top: 8, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => formatPHP(v)}
                />
                <Bar dataKey="assistance" stackId="a" fill="var(--color-primary)" radius={[0, 0, 0, 0]} />
                <Bar dataKey="operations" stackId="a" fill="var(--color-secondary)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card">
          <div className="flex items-center justify-between p-5 pb-3">
            ...
            <Link to="/assistance" className="text-xs font-medium text-primary hover:underline">View all</Link>
          </div>
          <ul className="divide-y divide-border">
            {pending.map((r) => (
              <li key={r.id} className="flex items-center gap-3 px-5 py-3">
                <Avatar className="h-9 w-9">
                  <AvatarFallback className="bg-warning/15 text-xs font-semibold text-warning">
        ...
        <div className="rounded-2xl border border-border bg-card">
          <div className="flex items-center justify-between p-5 pb-3">
            ...
            <Link to="/collection-events" className="text-xs font-medium text-primary hover:underline">Calendar</Link>
          </div>
          <ul className="divide-y divide-border">
            {upcoming.map((e) => (
          ...
          <ul className="divide-y divide-border">
            {recentMembers.map((m) => (
              <li key={m.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <Link to="/members/$id" params={{ id: m.id }} className="block truncate text-sm font-medium hover:text-primary">
      ...
      <div className="rounded-2xl border border-border bg-gradient-to-br from-primary/5 via-card to-secondary/5 p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link to="/announcements" className="inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline">
            Manage <ArrowRight className="h-3 w-3" />
          </Link>
        </div>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {pinned.map((a) => (
            <div key={a.id} className="rounded-xl border border-border bg-background/60 p-4 backdrop-blur">
